...
Begin VB.Form frmAvanEncajado 
   ...
   Begin VB.CommandButton cmdImprimirFortySeven 
      Caption         =   "Imprimir Formato Forty Seven"
      Height          =   495
      Left            =   4380
      TabIndex        =   5
      Top             =   4440
...
Private Sub cmdImprimirFortySeven_Click()
    REPORTE_FSE
End Sub

Private Sub REPORTE_FSE()
    ...
    '  Strsql = "EXEC UP_RPT_PACKINGLIST_FORMATO_FSE '" & varNum_Packing & "'"
    Ruta = App.Path & "\PACKINGLIST_FORTY_SEVEN.XLT"

    sql = "EXEC CF_MUESTRA_DETALLE_CAJAS_PACKINGLIST_FSE  " & vbNewLine
    sql = sql & "@OPCION='" & 2 & "'" & vbNewLine
    sql = sql & ",@NUM_PACKING='" & "" & "'" & vbNewLine
    sql = sql & ",@COD_CLIENTE='" & vCod_Cliente & "'" & vbNewLine
    sql = sql & ",@COD_PURORD='" & vCod_PurOrd & "'" & vbNewLine
    ...
    oo.Run "ReportePO", CargarRecordSetDesconectado(sql, cCONNECT)
    Set oo = Nothing
    
    Exit Sub

hand:
    ErrorHandler Err, "IMPRESION DE REPORTE FORTY SEVEN"
End Sub
...
Sub Reporte()
    ...
    Set oo = CreateObject("excel.application")
    oo.Workbooks.Open App.Path & "\AvanceEncajado.xlt"
