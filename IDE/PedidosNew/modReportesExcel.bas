...
Public Sub SoloNumeros(ByVal pTextbox As TextBox, _
    ...
    Else
        ...
        If iPos > 0 And pConDecimales Then If Len(Mid(pTextbox, iPos)) > pNumDecimales Then If InStr(pTextbox.SelStart + 1, pTextbox, ".") = 0 Then pKeyAscii = 0
...
Public Sub ErrorHandler(ByVal pErr As ErrObject, ByVal pProcedure As String)
    ...
    sMsg = pProcedure & " : " & Format(Now, "dd/mm/yyyy - hh:mm:ss") & Chr(13) & Chr(13) & "Número : " & pErr.Number & Chr(13) & "Descripción : " & pErr.Description & Chr(13) & "Fuente : " & pErr.source & Chr(13)
    Err.Clear
    MsgBox sMsg, vbCritical, App.Title
End Sub

Sub GeneraReportes(pTipo As TipoRep, _
                   Optional pMes As String, _
                   Optional pFabrica As String, _
                   Optional pCliente As String, _
                   Optional pRegistros As ADODB.Recordset)
