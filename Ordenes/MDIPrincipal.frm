...
Begin VB.MDIForm MDIPrincipal 
   ...
   Begin ActiveToolBars.SSActiveToolBars SSActiveToolBars1 
      ...
      _ExtentY        =   741
      _Version        =   327680
      Tools           =   "MDIPrincipal.frx":0442
      ToolBars        =   "MDIPrincipal.frx":045A
   End
...
End If
If vemp = "03" Then SSActiveToolBars1.ForeColor = RGB(0, 0, 153)
...
Dim count As Integer
    ...
    'strSql = "EXEC SM_MUESTRA_APLICACIONES_POR_USUARIO '" & vusu & "', '" & vemp & "'"
...
Private Sub BuildOptions()
    On Error GoTo Fail
    Dim rstOpts As New ADODB.Recordset, tooMenu As ActiveToolBars.SSTool, sTit As String
    
    sTit = "Cargar Menu"
    
'    Strsql = "SM_MUESTRA_OPCIONES_PERFIL_EMPRESA '" & vper & "', '" & vemp & "'"
    'strSql = "EXEC SM_MUESTRA_OPCIONES_POR_USUARIO '" & vusu & "', '" & vemp & "'"
...
'    Set rstOpts = CargarRecordSetDesconectado(Strsql, sconnect)
     ...
     With Comando
        .ActiveConnection = sconnect 'asocio el comando a la conexion
        .CommandType = adCmdStoredProc ' le digo q es un stored
        .CommandText = "SM_MUESTRA_OPCIONES_PERFIL_EMPRESA" ' el nombre del stored
        .Parameters.Append .CreateParameter("@Cod_Perfil", adChar, adParamInput, 4, vper)  ' variable de entrada
        .Parameters.Append .CreateParameter("@Cod_Empresa", adChar, adParamInput, 2, vemp)
        .CommandTimeout = 3600 '' 5 minutos
    End With
    
    With rstOpts
        .CursorLocation = adUseClient
        .Open Comando, , adOpenForwardOnly
        Set Comando.ActiveConnection = Nothing
        Set Comando = Nothing
        Set .ActiveConnection = Nothing
    End With

    If rstOpts.RecordCount > 0 Then
        With rstOpts
            .MoveFirst
            Do While Not .EOF
                AgregaOpcionAlMenu rstOpts.Fields("COD_APLICACION").value, _
                                   rstOpts.Fields("NIVEL").value, _
                                   rstOpts.Fields("COD_PADRE").value, _
                                   rstOpts.Fields("COD_OPCION").value, _
                                   rstOpts.Fields("DES_OPCION").value, _
                                   IIf(IsNull(rstOpts.Fields("DES_OPCION_ENG").value), "", rstOpts.Fields("DES_OPCION_ENG").value)
...
Private Sub AgregaOpcionAlMenu(ByVal sAplicacion As String, _
                               ByVal iNivel As Integer, _
                               ByVal sCodOpcionPadre As String, _
                               ByVal sCodOpcion As String, _
                               ByVal sDesOpcion As String, _
                               ByVal sDesOpcionIngles As String)
    On Error GoTo SALTO_ERROR
    
    Dim tooMenu As ActiveToolBars.SSTool

    If iNivel = 1 Then
        Set tooMenu = SSActiveToolBars1.Tools(sAplicacion).Menu.Tools.Add(sCodOpcion, ssTypeButton)
    Else
        Set tooMenu = SSActiveToolBars1.Tools(sCodOpcionPadre)
        tooMenu.Type = ssTypeMenu
        Set tooMenu = tooMenu.Menu.Tools.Add(sCodOpcion, ssTypeButton)
    End If
        
    tooMenu.Name = IIf(iLanguage = 1, sDesOpcion, sDesOpcionIngles)
    tooMenu.DisplayStyle = ssDisplayImageAndText
    Exit Sub

SALTO_ERROR:
        If Err.Number = 40002 Then
            MsgBox "No se encontró el Menu Superior " & UCase(sCodOpcionPadre) & " para" & vbNewLine & _
                   "la opcion " & UCase(sCodOpcion) & " (Nivel " & iNivel & ")." & vbNewLine & vbNewLine & _
                   "Verificar que este exista y su Nivel sea menor al de la opcion del Menu Superior......", _
                    vbCritical + vbOKOnly, "Mostrar Opcion en Menu (" & sCodOpcion & ")"
        Else
            MsgBox Err.Description, vbCritical + vbOKOnly, _
            "Mostrar Opcion en Menu (" & UCase(sCodOpcion) & ")"
        End If
End Sub
...
Fin:
        If Err.Number = 40002 Then
            MsgBox "No se encontró el Menu Superior " & rstOpcion!cod_padre & _
                   " para la opcion " & rstOpcion!Cod_Opcion & " (Nivel " & rstOpcion!nivel & ")" & _
                   " Verificar que este exista y su Nivel sea menor al de la opcion", _
            vbCritical + vbOKOnly, "Mostrar Opcion en Menu (" & rstOpcion!Cod_Opcion & ")"
        Else
            MsgBox Err.Description, vbCritical + vbOKOnly, _
            "Mostrar Opcion en Menu (" & rstOpcion!Cod_Opcion & ")"
        End If
End Sub
