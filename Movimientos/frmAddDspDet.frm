...
'                End If
'            End If
            txtCod_Barra = Trim(txtCod_Barra.Text)
            sPrefMaq = Left(txtCod_Barra.Text, 2)
            sCodRollo = Mid(txtCod_Barra.Text, 3, 6)
        Case optDirecto
    ...
    '        sCodRollo = Left(txtCodigo_Rollo, 6)
...
Sub ValidaDatos()
    ...
    Archivo = "peso"
    Ruta = "c:\" & "BALANZA\" & Archivo & ".txt"
    txtPeso.Text = ""
    Call AbrirArchivo(Ruta, txtPeso)
End Sub
...
Sub AbrirArchivo(Ruta As String, Texto As TextBox)
    ...
    Texto.Text = txt
    If Len(RTrim(LTrim(Texto.Text))) > 1 Then
     
    Texto.Text = Left(RTrim(txt), 4)
