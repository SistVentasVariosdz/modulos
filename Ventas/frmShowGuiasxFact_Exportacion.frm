...
Begin VB.Form frmShowGuiasxFact_Prendas 
   ...
   Begin VB.Frame FraBuscar 
      ...
      Begin FunctionsButtons.FunctButt FunctButt1 
         ...
         _ExtentX        =   2011
         _ExtentY        =   1005
         Custom          =   "0~0~BUSCAR~True~True~&Buscar~0~0~~~0~True~False~&Buscar~"
         Orientacion     =   0
         Style           =   1
         Language        =   0
         TypeImageList   =   0
...
Private Sub FunctButt2_ActionClick(ByVal Index As Integer, ByVal ActionType As Integer, ByVal ActionName As String)
    ...
    Case "AUTORIZARPAGO"
        ...
        Autorizar
    Case "PACKINGATRASADOS"
        Load frmPackingAtrasados
        frmPackingAtrasados.sCod_almacen = Left(Cbo_Almacen, 2)
        frmPackingAtrasados.Buscar
        frmPackingAtrasados.Show vbModal
        Set frmPackingAtrasados = Nothing
    Case "SALIR"
       Unload Me
    End Select
End Sub
...
Select Case ColIndex
  Case Is = GridEX1.Columns("Sel").Index
      ...
      If Trim(GridEX1.Value(GridEX1.Columns("Cod_Tipo_Transporte").Index)) <> "" Then
      ssql = VBsprintf(ssql, Left(Cbo_Almacen, 2), _
                       ...
                       GridEX1.Value(GridEX1.Columns("Pie_Factura2").Index), IIf(GridEX1.Value(GridEX1.Columns("Sel").Index) = 0, "P", "A"), GridEX1.Value(GridEX1.Columns("COD_ESTCLI").Index), GridEX1.Value(GridEX1.Columns("Fecha").Index), GridEX1.Value(GridEX1.Columns("Cod_Vendor").Index), GridEX1.Value(GridEX1.Columns("Cod_Class").Index), GridEX1.Value(GridEX1.Columns("Num_Embarque").Index), vusu, GridEX1.Value(GridEX1.Columns("Por_Comision").Index), GridEX1.Value(GridEX1.Columns("imp_Desaduanaje").Index), GridEX1.Value(GridEX1.Columns("Imp_Transporte_Pais_Destino").Index), FixNulos(GridEX1.Value(GridEX1.Columns("Cod_Tipo_Transporte").Index), vbString))
...
Private Sub txtcod_tipo_KeyPress(KeyAscii As Integer)
    If KeyAscii = vbKeyReturn Then
        KeyAscii = 0
        BuscaTipoTransporte 1
        SendKeys "{TAB}"
    End If
End Sub


Public Sub BuscaTipoTransporte(Opcion As String)
Dim rstAux As ADODB.Recordset

    strSQL = "SELECT Cod_TipoTransporte,Des_TipoTransporte FROM TG_Tipo_Transporte WHERE "
    
    txtcod_tipo = Trim(txtcod_tipo)
    txtdes_tipo = Trim(txtdes_tipo)
    
    Select Case Opcion
    Case 1: strSQL = strSQL & "Cod_TipoTransporte like '%" & txtcod_tipo & "%'"
    Case 2: strSQL = strSQL & "Des_TipoTransporte LIKE '%" & txtdes_tipo & "%'"
    End Select
    ...
    frmBusqGeneral3.gexLista.Columns("Cod_TipoTransporte").Width = 700
    frmBusqGeneral3.gexLista.Columns("Des_TipoTransporte").Width = 2000
    
    frmBusqGeneral3.gexLista.Columns("Cod_TipoTransporte").Caption = "Tipo"
    frmBusqGeneral3.gexLista.Columns("Des_TipoTransporte").Caption = "Descrip."
...
Private Sub CargarDatos()
    ...
    txtdes_tipo.Text = FixNulos(DevuelveCampo("select Des_TipoTransporte FROM TG_Tipo_Transporte where Cod_TipoTransporte = '" & FixNulos(GridEX1.Value(GridEX1.Columns("Cod_Tipo_Transporte").Index), vbLong) & "'", cCONNECT), vbString)
    ...
    txtcod_tipo.Text = FixNulos(GridEX1.Value(GridEX1.Columns("Cod_Tipo_Transporte").Index), vbString)
    txtdes_tipo.Text = FixNulos(DevuelveCampo("select Des_TipoTransporte FROM TG_Tipo_Transporte where Cod_TipoTransporte = '" & FixNulos(GridEX1.Value(GridEX1.Columns("Cod_Tipo_Transporte").Index), vbLong) & "'", cCONNECT), vbString)
...
Dim ssql As String
    ...
    GridEX1.Value(GridEX1.Columns("Cod_Tipo_Transporte").Index) = txtcod_tipo.Text
      ...
      ssql = VBsprintf(ssql, Left(Cbo_Almacen, 2), _
                       ...
                       IIf(GridEX1.Value(GridEX1.Columns("Sel").Index) = 0, "P", "A"), GridEX1.Value(GridEX1.Columns("COD_ESTCLI").Index), GridEX1.Value(GridEX1.Columns("Fecha").Index), GridEX1.Value(GridEX1.Columns("Cod_Vendor").Index), GridEX1.Value(GridEX1.Columns("Cod_Class").Index), GridEX1.Value(GridEX1.Columns("Num_Embarque").Index), vusu, GridEX1.Value(GridEX1.Columns("Por_comision").Index), GridEX1.Value(GridEX1.Columns("imp_Desaduanaje").Index), GridEX1.Value(GridEX1.Columns("Imp_Transporte_Pais_Destino").Index), FixNulos(GridEX1.Value(GridEX1.Columns("Cod_Tipo_Transporte").Index), vbString))
...
On Error GoTo errx
    ...
    ssql = VBsprintf(ssql, "I", vusu, Left(Cbo_Almacen, 2), _
            ...
            GridEX1.Value(GridEX1.Columns("Imp_Comision").Index), txtCod_TipoFact.Text, _
            GridEX1.Value(GridEX1.Columns("Num_Partida_Arancelaria").Index), _
            GridEX1.Value(GridEX1.Columns("Sec_Partida_Arancelaria").Index), _
            GridEX1.Value(GridEX1.Columns("Num_Categoria_Internacional").Index))
...
Private Sub Verificacion_Packing()

On Error GoTo drDepurar

Dim ssql As String

ExecuteSQL cCONNECT, "VENTAS_VERIFICACION_PACKING '" & Left(Cbo_Almacen, 2) & "'"
