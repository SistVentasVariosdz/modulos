...
Public sQuery As String, iCampo As Integer
...
DGridLista.Columns(1).Width = 3500
Exit Sub
Cargar_DatosErr:
...
If Not Rs_Carga.EOF And Not Rs_Carga.BOF Then
    With oParent
        ...
        If Rs_Carga.Fields.Count > 1 Then
            'OldCond: Rs_Carga.Fields.Count = 2
...
Private Sub DGridLista_KeyPress(KeyAscii As Integer)
    ...
    Else
        ...
        Set Rs = DGridLista.DataSource
        Call BuscaCampo(Rs, Rs(iCampo).Name, UCase(Chr(KeyAscii)))
        'DGridLista.DataSource.moveto Rs.Bookmark
...
Sub Cargar_DatosSeguridad()
On Error GoTo Cargar_DatosErr
Rs_Carga.ActiveConnection = cSEGURIDAD
Rs_Carga.CursorType = adOpenStatic
Rs_Carga.CursorLocation = adUseClient
Rs_Carga.LockType = adLockReadOnly
Rs_Carga.Open sQuery
